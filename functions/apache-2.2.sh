...
sed -i "s@^DocumentRoot.*@DocumentRoot \"$home_dir/public_html\"@" $apache_install_dir/conf/httpd.conf
sed -i "s@^<Directory \"$apache_install_dir/htdocs\">@<Directory \"$home_dir/public_html\">@" $apache_install_dir/conf/httpd.conf
...
<VirtualHost *:$TMP_PORT>
    ...
    ServerName $TMP_IP 
    ErrorLog "$logs_dir/error_apache.log"
    CustomLog "$logs_dir/access_apache.log" common
