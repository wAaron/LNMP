...
if [ "$Nginx_version" == '1' ];then
	...
	if [ `getconf WORD_BIT` == 32 ] && [ `getconf LONG_BIT` == 64 ] ;then
		./configure $nginx_configure_arguments --add-module=../ngx_pagespeed-1.9.32.1-beta --with-cc-opt='-DLINUX=2 -D_REENTRANT -D_LARGEFILE64_SOURCE -pthread'
	else
		./configure $nginx_configure_arguments --add-module=../ngx_pagespeed-1.9.32.1-beta --with-cc-opt='-DLINUX=2 -D_REENTRANT -D_LARGEFILE64_SOURCE -march=i686 -pthread'
	fi
