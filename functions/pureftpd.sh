...
update-rc.d pureftpd defaults"
	...
	sed -i 's/tmppasswd/'$conn_ftpusers_dbpwd'/' public_ftp/config.php
	sed -i "s/myipaddress.com/`echo $local_IP`/" public_ftp/config.php
	sed -i 's@\$DEFUserID.*;@\$DEFUserID = "501";@' public_ftp/config.php
	sed -i 's@\$DEFGroupID.*;@\$DEFGroupID = "501";@' public_ftp/config.php
	sed -i 's@iso-8859-1@UTF-8@' public_ftp/language/english.php
	/bin/cp ../conf/chinese.php public_ftp/language/
	sed -i 's@\$LANG.*;@\$LANG = "chinese";@' public_ftp/config.php
	rm -rf  public_ftp/install.php
	mv public_ftp $home_dir/
	cd ..
