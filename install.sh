...
mkdir -p $home_dir/public_html $logs_dir $lnmp_dir/{src,conf}
...
do
		...
		if [ "$phpMyAdmin_yn" == 'y' ];then
		        [ -d "$home_dir/public_html/phpMyAdmin" ] && echo -e "\033[31mThe phpMyAdmin already installed! \033[0m" && phpMyAdmin_yn=n && break
		fi
                break
        fi
done
...
# index example
if [ ! -e "$home_dir/public_html/index.html" -a "$Web_yn" == 'y' ];then
	. functions/test.sh
...
[ "$FTP_yn" == 'y' ] && echo -e "`printf "%-32s" "Pure-FTPd php manager dir:"`\033[32m$home_dir/public_ftp\033[0m"
[ "$FTP_yn" == 'y' ] && echo -e "`printf "%-32s" "Ftp User Control Panel url:"`\033[32mhttp://$local_IP/public_ftp\033[0m"
[ "$phpMyAdmin_yn" == 'y' ] && echo -e "\n`printf "%-32s" "phpMyAdmin dir:"`\033[32m$home_dir/public_html/phpMyAdmin\033[0m"
